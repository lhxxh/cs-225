...
int LPHashTable<K, V>::findIndex(const K& key) const
{
    ...
    for(;retval<size;retval++){
      if(should_probe[retval]==true){
      if(key==table[retval]->first)
      return retval;
      }
    }
    return -1;
     ...
     */
     ...
     for(size_t i=0;i<size;++i){
       if(should_probe[i]==true){
         unsigned idx=hash(table[i]->first,newsize);
         newtable[idx]=new pair<K,V>(table[i]->first,table[i]->second);
