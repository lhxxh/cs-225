...
void SCHashTable<K, V>::insert(K const& key, V const& value)
{
     ...
     */
     ...
     pair<K,V> p=std::make_pair(key,value);
...
void SCHashTable<K, V>::resizeTable()
{
     ...
     */
     ...
     for(size_t counter=0;counter<size;counter++){
       for(it=table[counter].begin();it!=table[counter].end();++it){
         unsigned newidx=hash(it->first,newsize);
         newtable[newidx].push_front(pair<K,V>(it->first,it->second));
       }
     }
     delete[] table;
     table=newtable;
