...
{
    /* Your code goes here! */
    string upper_word1,upper_word2;
    upper_word1.resize(word1.size());
    upper_word2.resize(word2.size());
    transform(word1.begin(),word1.end(),upper_word1.begin(),::toupper);
    transform(word2.begin(),word2.end(),upper_word2.begin(),::toupper);
    map<string,vector<string>>::const_iterator iter1=dict.find(upper_word1);
    map<string,vector<string>>::const_iterator iter2=dict.find(upper_word2);
    if(iter1==dict.end()||iter2==dict.end()){return false;}
    if(iter2->second==iter1->second){;return true;}
    else
    return false;
