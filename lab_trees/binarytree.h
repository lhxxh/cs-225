...
class BinaryTree
{
    ...
    private:
        ...
        /*helper function of mirror()*/
        void mirror(Node* next);
        /*helper function of isOrderedRecursiver*/
        void isOrderedRecursivehelper(std::stack<T>& stack,typename BinaryTree<T>::Node* node)const;
        void getpathhelper(std::vector<T>& vectorrecord,vector<vector<T> > &paths,Node* temp)const;
        int sumDistanceshelper(Node* node,int depth)const;
};
