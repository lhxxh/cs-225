...
class KDTree
{
  ...
  private:
     ...
     */
    KDTreeNode* KDtree_helper(vector<Point<Dim>>& newPoints,int left,int right,int dimension);
    Point<Dim>  selectmedian(vector<Point<Dim>>& newPoints,int left,int right,int median,int dimension);
    int partition(vector<Point<Dim>>& newPoints,int left,int right,int dimension);
    void destroy_(KDTreeNode* node);
    KDTreeNode* copy_(const KDTreeNode* node);
    Point<Dim> findsmallerstrec(KDTreeNode* node,const Point<Dim>& query,int dimension) const;
    double twopointdis(const Point<Dim>& first,const Point<Dim>& second) const;
};
