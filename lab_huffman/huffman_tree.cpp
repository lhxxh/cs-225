                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
     ...
     if(mergeQueue.empty()==0&&singleQueue.empty()==0){
     if(singleQueue.front()->freq.getFrequency()>mergeQueue.front()->freq.getFrequency()){
       retnode=mergeQueue.front();
       mergeQueue.pop();
     }
     else{
       retnode=singleQueue.front();
       singleQueue.pop();
     }
   }
   else if(mergeQueue.empty()==1&&singleQueue.empty()==0){
     retnode=singleQueue.front();
     singleQueue.pop();
   }
   else if(mergeQueue.empty()==0&&singleQueue.empty()==1){
     retnode=mergeQueue.front();
     mergeQueue.pop();
   }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for(const Frequency& it:frequencies)
       singleQueue.push(new TreeNode(it));
      while(singleQueue.size()+mergeQueue.size()!=1){
        TreeNode* first_smallnode=HuffmanTree::removeSmallest(singleQueue,mergeQueue);
        TreeNode* second_smallnode=HuffmanTree::removeSmallest(singleQueue,mergeQueue);
        int merge=first_smallnode->freq.getFrequency()+second_smallnode->freq.getFrequency();
        TreeNode* newmergenode=new TreeNode(merge);
        newmergenode->left=first_smallnode;
        newmergenode->right=second_smallnode;
        mergeQueue.push(newmergenode);
      }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     readTreehelper(newhead,bfile);

    return newhead;
}
void HuffmanTree::readTreehelper(TreeNode*& node,BinaryFileReader& bfile){
if(!bfile.hasBits()){return;}
if(bfile.getNextBit()==1){
  char newchar=bfile.getNextByte();
  Frequency newfreq(newchar,0);
  node=new TreeNode(newfreq);
  return;
}
else{
  node=new TreeNode(0);
  readTreehelper(node->left,bfile);
  readTreehelper(node->right,bfile);
}
