 ...
 */
 ...
 size_t idx_helper(const std::vector<T>& elements, const C& val, int startPoint, int endPoint){
   T mid = elements[(startPoint + endPoint) / 2];
   if((val > elements[startPoint]) && (val < elements[endPoint]) && (startPoint == (endPoint -1))){
     return endPoint;
   }
   ...
   if(mid < val){
     return idx_helper(elements, val, (startPoint + endPoint) / 2, endPoint);
   }
   if(mid > val){
     return idx_helper(elements, val, startPoint, (startPoint + endPoint) / 2);
   }
   return 0;
 }
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
  ...
  return idx_helper(elements, val, 0, i-1);
